unset INSTALL_LOCAL_VERSION
unset INSTALL_LOCAL_DIR

: ${INSTALL_LOCAL_TEST_DIR:="${BATS_TMPDIR}/install-local"}
...
if [ "$INSTALL_LOCAL_ROOT" != "${INSTALL_LOCAL_TEST_DIR}/root" ]; then
  ...
  PATH="${INSTALL_LOCAL_TEST_DIR}/bin:$PATH"
  PATH="${BATS_TEST_DIRNAME}/../libexec:$PATH"
  PATH="${BATS_TEST_DIRNAME}/libexec:$PATH"
  PATH="${INSTALL_LOCAL_ROOT}/shims:$PATH"
  export PATH
fi

teardown() {
  rm -rf "$INSTALL_LOCAL_TEST_DIR"
}
...
flunk() {
  ...
  } | sed "s:${INSTALL_LOCAL_TEST_DIR}:TEST_DIR:g" >&2
  return 1
}
...
path_without() {
  ...
  for found in $(which -a "$exe"); do
    found="${found%/*}"
    if [ "$found" != "${INSTALL_LOCAL_ROOT}/shims" ]; then
      alt="${INSTALL_LOCAL_TEST_DIR}/$(echo "${found#/}" | tr '/' '-')"
      mkdir -p "$alt"
